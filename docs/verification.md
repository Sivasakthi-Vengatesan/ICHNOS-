...
TraceLake-V uses the **Z3 Theorem Prover** developed by Microsoft Research.

When TraceLake-V returns:
...
### `UNKNOWN`
The transformation contains blackbox or unsupported operations outside the symbolic model (e.g. unconstrained UDFs, non-deterministic RNG, external API calls). TraceLake-V will **never fake a verification result**.

## 2. Limitations

- **Bounded Symbolic Model**: Guarantees apply to transformations expressed in the TraceLake-V IR. Arbitrary Python bytecodes or external library binaries are not verified unless transpiled to IR.
